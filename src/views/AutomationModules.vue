...
export default {
  ...
  setup() {
    ...
    const calculateOEE = (statusData, airData) => {
      ...
      try {
        // 1. Availability = Actual Running Time / Planned Production Time
        const plannedProductionTime = statusData.length; // Total time samples
        const downtime = statusData.filter(d => !d.Active || d.Error).length; // Time when machine is not active or has errors
        const actualRunningTime = plannedProductionTime - downtime;
        const availability = (actualRunningTime / plannedProductionTime) * 100;
    
        // 2. Performance = (Total Parts × Ideal Cycle Time) / Operating Time
        let performance = 0;
        if (airData?.length && actualRunningTime > 0) {
          const totalParts = airData.length;
          const idealCycleTime = OEE_CONFIG.idealCycleTime; // 60 seconds
          const theoreticalOutput = (actualRunningTime * 60) / idealCycleTime; // Convert to minutes
          performance = Math.min((totalParts / theoreticalOutput) * 100, 100); // Cap at 100%
        }
    
        // 3. Quality = Good Parts / Total Parts
        let quality = 0;
        const totalPartsProduced = airData?.length || 0;
        ...
        // Debug logging
        console.log('OEE Calculation Details:', {
          availability: {
          ...
          performance: {
            totalParts: airData?.length,
            idealCycleTime: OEE_CONFIG.idealCycleTime,
            theoreticalOutput: (actualRunningTime * 60) / OEE_CONFIG.idealCycleTime,
            value: performance.toFixed(1) + '%'
          },
          quality: {
            totalPartsProduced,
            defectiveParts: statusData.filter(d => d.Error).length,
            goodParts: totalPartsProduced - statusData.filter(d => d.Error).length,
    ...
    const fetchData = async () => {
      try {
        ...
        // Process XDK sensor data with millisecond precision
        if (xdkData?.length) {
          environmentData.value = {
            labels: xdkData.map(d => formatTimestamp(d.time)),
            datasets: [
              {
                label: 'Temperature (°C)',
          ...
          if (lightData?.length) {
            ...
            const processedLightData = {
              labels: [...new Set(lightData.map(d => formatTimestamp(d.time)))],
        ...
        // Process motion data with millisecond precision
        if (motionData?.acceleration?.length) {
          accelerationData.value = {
            labels: motionData.acceleration.map(d => formatTimestamp(d.time)),
            datasets: [
              {
                label: 'X-axis',
        ...
        if (motionData?.gyroscope?.length) {
          gyroscopeData.value = {
            labels: motionData.gyroscope.map(d => formatTimestamp(d.time)),
            datasets: [
              {
                label: 'X-axis',
        ...
        if (magneticResult?.length) {
          ...
          const processedMagneticData = {
            labels: [...new Set(magneticResult.map(d => formatTimestamp(d.time)))],
        ...
        // Update chart options to show more timestamps
        const commonAxisOptions = {
          x: {
            ...
            ticks: {
              ...
              padding: 10,
              autoSkip: true,
              maxTicksLimit: 10 // Adjust this value to show more/fewer timestamps
            }
          }
        };
        ...
        environmentOptions.value = {
          ...environmentOptions.value,
          scales: {
            ...environmentOptions.value.scales,
            ...commonAxisOptions
          }
        };
